document.addEventListener("DOMContentLoaded", function() {
  ...
  // --- HELPER: Contrast Checker (The Fix) ---
  ...
  function getContrastColor(h, s, l) {
    ...
    // Threshold of 128 is standard. > 128 means the background is light.
  ...
  function updateTheme() {
    ...
    // --- WHITE COLOR FIX ---
    // If input is white (L > 95%), switch to a "Pinkish White" base
    // This allows the other sections (Main/About) to have colors while Welcome stays light.
    if (hsl.l > 95) {
      hsl.h = 338; 
      hsl.s = 48;
      hsl.l = 90; // High lightness -> contrast checker will choose BLACK text
    }

    // 1. CALCULATE BACKGROUNDS
    
    // Main Color: Rotated +30deg
    const mainH = (hsl.h + 30) % 360; 
    const mainS = hsl.s; 
    const mainL = hsl.l;
    
    // Welcome: Uses the base color
    const welcomeH = hsl.h;
    const welcomeS = hsl.s;
    const welcomeL = hsl.l; 

    // About: Rotated -30deg
    let aboutH = (hsl.h - 30) % 360;
    if (aboutH < 0) aboutH += 360;
    const aboutL = 40; // Keeps about section relatively dark

    // Projects: Same Hue, Darker
    const projectsH = hsl.h;
    const projectsL = 15; // Keeps projects section very dark
    ...
    // This runs for EVERY section. If a section is light, text becomes black.
